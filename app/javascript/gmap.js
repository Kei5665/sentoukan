  ...
  if (gon.shops) {
    ...
    for (let i = 0; i < gon.shops.length; i++) {
      ...
      infoWindow[i] = new google.maps.InfoWindow({
        content: '<div class="card shadow-none">'+'<div class="card-body">'+'<h3 class="font-bold">'+ gon.shops[i]['name'] +'</h3>'+'<p class="mb-3 opacity-60">'+ gon.shops[i]['address'] +'</p>'+'<p class="mb-3 opacity-60">'+ gon.shops[i]['opening_hours'] +'</p>'+'<form action="/quests" accept-charset="UTF-8" method="post"><input value='+gon.shops[i]['id']+' type="hidden" name="q[shop_id]"><input value='+lat+' type="hidden" name="q[latitude]"><input value='+lng+' type="hidden" name="q[longitude]"><input type="submit" name="commit" value="ここに行く" class="button button-orange text-lg font-bold" data-disable-with="ここへ行く"></form>'+'</div>'+'</div>'
      });
      // マーカークリックで情報ウィンドウを表示
